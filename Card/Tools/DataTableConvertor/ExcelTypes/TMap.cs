using System;
using System.Collections.Generic;
using System.Text;
using System.IO;

namespace ExcelConvertor.ExcelTypes
{
    public class TMap<_TKey, _TValue> : IType
        where _TKey : IType, new()
        where _TValue : IType, new()
    {
        static _TKey _Tk = new _TKey();
        static _TValue _Tv = new _TValue();
        static string typeName = string.Format("map<{0},{1}>", _Tk.Name, _Tv.Name);

        public eTypeCode TypeCode { get { return eTypeCode.Map; } }
        public string Name { get { return typeName; } }
        public IType TKey { get { return _Tk; } }
        public IType TValue { get { return _Tv; } }
        public bool Write(BinaryWriter writer, string val)
        {
            // "{0,1}/{1.2}"
            //string[] mapEntities = val.Split('/');
            string[] mapEntities = val.Split(',');
            writer.Write(mapEntities.Length);

            //for (int i = 0; i < mapEntities.Length; ++i)
            //{
            //    string mapEntity = mapEntities[i];

            //    string[] tempElements1 =  mapEntity.Split('{');
            //    if (tempElements1.Length > 2 || tempElements1.Length < 2)
            //        return false;

            //    string[] tempElements2 = tempElements1[1].Split('}');
            //    if (tempElements2.Length > 2 || tempElements2.Length < 2)
            //        return false;

            //    string[] elements = tempElements2[0].Split(',');
            //    if (elements.Length > 2 || elements.Length < 2)
            //        return false;

            //    string keyVal = elements[0];
            //    string valueVal = elements[1];

            //    if (false == _Tk.Write(writer, keyVal))
            //        return false;

            //    if (false == _Tv.Write(writer, valueVal))
            //        return false;
            //}
            for (int i = 0; i < mapEntities.Length; ++i)
            {
                if (false == _Tv.Write(writer, mapEntities[i]))
                    return false;
            }
            return true;
        }
    }
}
